...
<div id="content">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <table class="table table-condensed table-stripped" ng-repeat="project in projects">
                <thead>
                <tr class="info">
                    <th colspan="4">
                ...
                <tbody>
                <tr ng-repeat="sandbox in sandboxesByProject[project.id] | sandboxFilter:options | orderBy:['title']">
                    <td class="col-xs-3"><i class="fa fa-lg fa-check text-success" ng-if="task.completed === 'Y'"></i> {{ sandbox.title }}</td>
                    <td class="col-xs-7" ng-class="'text-' + compareDates(task.due_date)">{{ sandbox.description }}</td>
                    <td class="col-xs-1">{{ sandbox.name_author }}</td>
