...
class Zeapps_project_deadlines extends ZeModel {
    ...
    public function get_all($where = array()){
        ...
        return $this->database()->select('*, zeapps_project_deadlines.id as id, zeapps_project_deadlines.title as title, zeapps_projects.title as project_title, zeapps_project_deadlines.due_date as due_date')->join('zeapps_projects', 'zeapps_project_deadlines.id_project = zeapps_projects.id', 'LEFT')->where($where)->table('zeapps_project_deadlines')->result();
    }
